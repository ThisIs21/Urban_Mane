...
const Bundle = () => {
  ...
  const fetchBundles = useCallback(async () => {
    try {
      ...
      console.error("Gagal fetch bundles:", err);
    } finally {
      setLoading(false);
    }
  }, [search]);
  ...
  // Fetch available products & services
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.error("Gagal memuat data produk/service:", err);
      }
    };
    loadData();
  }, []);
  ...
  const openEdit = (b) => {
    console.log("Opening edit for bundle:", b); // untuk debugging

    setEditingBundle(b);

    // Update form dengan data bundle
    setForm({
      name: b.name || '',
    ...
    setPhotoPreview(b.image || null);

    // Penting: Gunakan setTimeout agar state selesai di-update sebelum modal dibuka
    setTimeout(() => {
      setIsModalOpen(true);
    }, 10);
  };
  ...
  const addProduct = () => {
    ...
    if (form.products.some(p => p.productId === selectedProductId)) {
      return alert('Produk sudah ditambahkan');
    }
  ...
  const addService = () => {
    if (!selectedServiceId) return alert('Pilih layanan terlebih dahulu');
    const service = availableServices.find(s => s.id === selectedServiceId);
    if (!service) return;

    if (form.services.some(s => s.serviceId === selectedServiceId)) {
      return alert('Service sudah ditambahkan');
    }
  ...
  const removeProduct = (id) => {
    setForm(prev => ({
      ...prev,
      products: prev.products.filter(x => x.productId !== id)
    }));
  };
  ...
  const removeService = (id) => {
    setForm(prev => ({
      ...prev,
      services: prev.services.filter(x => x.serviceId !== id)
    }));
  };
  ...
  const handleSave = async () => {
    ...
    if (form.products.length === 0 && form.services.length === 0) {
      return alert('Bundle harus memiliki minimal 1 produk atau service');
    }
    if (!form.bundlePrice || form.bundlePrice <= 0) {
      return alert('Harga bundle harus lebih dari 0');
    }

    try {
      if (editingBundle) {
        await bundleService.updateBundle(editingBundle.id, form);
      } else {
        await bundleService.createBundle(form);
      }
      setIsModalOpen(false);
      fetchBundles();
      alert(editingBundle ? 'Bundle berhasil diupdate' : 'Bundle berhasil dibuat');
    } catch (err) {
      console.error(err);
      alert(err.response?.data?.error || 'Gagal menyimpan bundle');
    }
  };
    ...
    <>
      ...
      <div className="adm-page">
              ...
              <tbody>
                {loading ? (
                  Array.from({ length: 3 }).map((_, i) => (
                    <tr key={i}>
                      {[40, 120, 90, 60, 100, 60, 80].map((w, j) => (
                        <td key={j}><span className="skel" style={{ width: w, height: 13 }} /></td>
                      ))}
                    </tr>
                  ))
                ) : filtered.length === 0 ? (
                  <tr><td colSpan="7"><div className="adm-empty">Tidak ada paket.</div></td></tr>
                ) : (
                  filtered.map(b => (
                    <tr key={b.id}>
                      <td>
                        <div className="adm-thumb">
                          {b.image ? (
                            <img 
                              src={bundleService.getImageUrl(b.image)} 
                              alt={b.name} 
                      ...
                      <td>
                        <span className={`adm-badge ${b.stock < 5 ? 'stock-low' : 'stock-ok'}`}>
                          {b.stock} pcs
                        </span>
                      </td>
                      <td>
                        <span style={{ color: 'var(--t2)' }}>
                          {b.products?.length || 0} produk, {b.services?.length || 0} layanan
                        </span>
                      </td>
                      <td>
                        <span className={`adm-badge ${b.isActive ? 'active' : 'inactive'}`}>
                      ...
                      <td className="adm-td-center">
                        <div className="adm-action-cell">
                          ...
                          <button 
                            className="adm-btn-stock" 
                            onClick={() => openStockModal(b)}
                            style={{ backgroundColor: '#3B82F6', color: 'white', padding: '6px 12px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '11px', fontWeight: '600', marginLeft: '4px' }}
                          >
                            Stok
                          </button>
                          <button 
        ...
        <Modal 
          ...
          title={editingBundle ? 'Edit Paket' : 'Tambah Paket Baru'}
        >
          <div className="adm-modal-body">
            <div className="adm-field">
              <label className="adm-field-label">Nama Paket</label>
              <input 
            ...
            <div className="adm-field-row">
              <div className="adm-field">
                <label className="adm-field-label">Harga Paket (Rp)</label>
                <input 
                  className="adm-field-input" 
                  type="number" 
                  value={form.bundlePrice} 
                  onChange={e => set('bundlePrice', parseInt(e.target.value) || 0)} 
                  placeholder="0" 
                />
              </div>
              <div className="adm-field">
        ...
        <Modal isOpen={deleteOpen} onClose={() => setDeleteOpen(false)} title="Hapus Paket">
          <p className="adm-delete-msg">
            Hapus paket <span className="adm-delete-name">"{deleteTarget?.name}"</span>?
          </p>
          <div className="adm-modal-footer">
        ...
        <Modal 
          ...
          title={`Kelola Stok - ${stockBundle?.name}`}
        >
          <form onSubmit={handleStockUpdate}>
            {/* Isi stock modal kamu yang lama tetap sama */}
            <div className="adm-modal-body">
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button 
                    ...
                    style={{
                      ...
                      border: stockAction === 'add' ? '2px solid #3B82F6' : '1px solid #D1D5DB',
                      backgroundColor: stockAction === 'add' ? '#EFF6FF' : 'white',
                      color: stockAction === 'add' ? '#3B82F6' : '#666',
                  ...
                  <button 
                    ...
                    style={{
                      flex: 1, padding: '10px', borderRadius: '6px',
                      border: stockAction === 'remove' ? '2px solid #EF4444' : '1px solid #D1D5DB',
                      backgroundColor: stockAction === 'remove' ? '#FEF2F2' : 'white',
                      color: stockAction === 'remove' ? '#EF4444' : '#666',
                      fontWeight: '600', cursor: 'pointer'
                    }}
                  >
                    ➖ Kurangi Stok
              ...
              <div className="adm-field">
                ...
                <input 
                  ...
                  placeholder="0" 
                  min="1" 
                  max={stockAction === 'remove' ? stockBundle?.stock : 9999} 
                />
              </div>
              ...
              <div style={{ marginTop: '20px', padding: '12px', backgroundColor: '#F0FDF4', borderRadius: '8px', borderLeft: '4px solid #10B981' }}>
                <p style={{ fontSize: '13px', color: '#065F46', margin: '0' }}>
                  Stok baru: <strong>
                    {stockAction === 'add' 
                      ? parseInt(stockQuantity || 0) + parseInt(stockBundle?.stock || 0) 
                      : parseInt(stockBundle?.stock || 0) - parseInt(stockQuantity || 0)
                    } pcs
                  </strong>
                </p>
              </div>
